...
REFERENCE_FILES = {
    ...
    "1st_secandory": {
        ...
        "الرياضيات": [
            {"label": "كتاب الرياضيات - 1", "file": "math1.pdf"},
            {"label": "كتاب الرياضيات - 2", "file": "math2.pdf"}
        ],
    ...
    "2nd_secandory_L": {
        ...
        "علم النفس": [{"label": "كتاب علم النفس", "file": "psychology.pdf"}],
        "المطالعة والإنشاء": [{"label": "كتاب المطالعة والإنشاء", "file": "Reading and writing.pdf"}],
        "البلاغة": [{"label": "كتاب البلاغة", "file": "Rhetoric.pdf"}],
        "علم الاجتماع": [{"label": "كتاب علم الاجتماع", "file": "sociology.pdf"}],
        "الإحصاء": [{"label": "كتاب الإحصاء", "file": "statistics.pdf"}]
    },
    "3rd_secandory_S": {
    ...
    "3rd_secandory_L": {
        ...
        "التربية الإسلامية": [{"label": "كتاب التربية الإسلامية", "file": "islamic.pdf"}],
        "تقنية المعلومات": [{"label": "كتاب تقنية المعلومات", "file": "IT.pdf"}],
        "النقد الأدبي": [{"label": "كتاب النقد الأدبي", "file": "Literary_criticism.pdf"}],
        "الأدب والنصوص": [{"label": "كتاب الأدب والنصوص", "file": "Literature.pdf"}],
        "النحو والصرف والإملاء": [{"label": "كتاب النحو والصرف والإملاء", "file": "naho.pdf"}],
        "الفلسفة": [{"label": "كتاب الفلسفة", "file": "Philosophy.pdf"}],
        "علم النفس": [{"label": "كتاب علم النفس", "file": "psychology.pdf"}],
...
def build_year_reference_overview(study_year):
...
def build_ai_prompt(subject, query, study_year, is_english):

    reference_note = f"اعتمد فقط على الكتب المرفقة في الواجهة الخاصة بمرحلة {study_year}، وإذا كانت المعلومة غير موجودة فيها قل بوضوح إنها غير متاحة في المنهج الحالي."

    if is_english:

        system_message = (
            "أنت 'أستاذ ليبي عبقري' اسمك 'افهمها وفهمني'. تخصصك المنهج الليبي. "
            "شخصيتك قوية، محبوبة، ودمك خفيف. تعامل الطلاب كخوتك الصغار، وخليك محترم حتى لو الكلام كان فيه ألفاظ غير لائقة."
        )

        prompt = f"""
مرحباً يا بطل! نبيك تشرحلي موضوع ({query}) في مادة ({subject}) لصف ({study_year}) بالمنهج الليبي.

خطة الشرح المطلوبة:
1. ابدأ بمقدمة قصيرة وواضحة تربط الموضوع بالمنهج وبالهدف من دراسته.
2. اشرح الفكرة خطوة خطوة باللهجة الليبية البيضاء، وعرّف أي مصطلح جديد بطريقة سهلة.
3. استعمل أمثلة من الواقع الليبي باش الفكرة تثبت.
4. لو فيه قوانين، رموز، أو مصطلحات علمية، اكتبها بشكل صحيح وواضح.
5. لو الموضوع فيه باب أو وحدة، ركز على الزبدة، أهم التعاريف، والأفكار اللي تتكرر في الامتحانات.
6. خلي الشرح طويل ومفيد، مش مختصر: اكتب على الأقل 6 إلى 8 فقرات منظمة بعناوين واضحة.
7. أضف قسم "أخطاء شائعة" يوضح الحاجات اللي الطالب يلخبط فيها عادة.
8. اختم بخلاصة عملية ونصائح مذاكرة سريعة، وبعدها 3 أسئلة Quiz قصيرة.

{reference_note}

رد عليا بصيغة JSON فقط:

{{

 "explanation": "شرح طويل ومنظم بعناوين وفقرات وأمثلة وخلاصة...",
...
"""
        ...
        system_message = (
            "أنت 'أستاذ ليبي خبير' خريج جامعة طرابلس/بنغازي، اسمك 'افهمها وفهمني'. "
            "أسلوبك بسيط وواضح، وتستخدم أمثلة من الشارع الليبي (الضي، السوق، الكورة) بدون تعقيد."
        )

        prompt = f"""
يا أستاذ، نبيك تشرحلي موضوع ({query}) في مادة ({subject}) لصف ({study_year}) حسب كتاب المنهج الليبي الرسمي.

خطة الشرح المطلوبة:
1. ابدأ بمقدمة ودودة توضح أهمية الموضوع في المنهج.
2. اشرح الفكرة بالتدرج من السهل إلى الأصعب، وخلي الشرح مرتب وواضح.
3. إذا الموضوع فيه قانون أو مصطلح علمي، فسّره ببساطة مع مثال عملي.
4. اربط الشرح بحياة الطالب اليومية في ليبيا حتى يفهم بسرعة.
5. لو فيه باب أو وحدة، شدد على الخلاصة والنقاط اللي تتكرر في الامتحانات.
6. خلي الشرح طويل ومفيد، مش مختصر: اكتب على الأقل 6 إلى 8 فقرات منظمة بعناوين واضحة.
7. أضف قسم "أخطاء شائعة" يوضح الحاجات اللي الطالب يلخبط فيها عادة.
8. اختم بخلاصة مختصرة و3 أسئلة اختيار من متعدد لقياس الفهم.
...
{{
 ...
 "explanation": "شرح طويل ومفصل بعناوين وفقرات وأمثلة وخلاصة...",
...
def inject_curriculum_data():
    ...
    if not current_user.is_authenticated:
        ...
        return {"references_map": {}, "curriculum_overview": []}

    study_year = current_user.study_year

    return {

        "references_map": build_references_map(study_year),

        "curriculum_overview": build_year_reference_overview(study_year)
...
def ai_room():
    ...
    if request.method == 'POST':
        ...
        system_message, prompt = build_ai_prompt(subject, query, current_user.study_year, is_english)
